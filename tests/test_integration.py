...
import spaceload.daemon.server as _m
from pathlib import Path

# Override module-level path constants before starting
_m._SPACELOAD_DIR = Path({str(sock_path.parent)!r})
_m._SOCKET_PATH = Path({str(sock_path)!r})
_m._PID_PATH = Path({str(pid_path)!r})

from spaceload.daemon.server import RecorderDaemon
