...
from spaceload.shell.hooks import get_hook_script, ZSH_HOOK, BASH_HOOK
...
class TestGetHookScript:
    def test_zsh_hook(self):
        ...
        assert "_spaceload_preexec" in script
    ...
    def test_bash_hook(self):
        ...
        assert "_spaceload_debug_trap" in script
...
class TestZshHookContent:
    def test_checks_socket_exists(self):
        assert '[[ -S "$_spaceload_socket" ]]' in ZSH_HOOK
...
class TestBashHookContent:
    def test_checks_socket_exists(self):
        assert '[[ -S "$_spaceload_socket" ]]' in BASH_HOOK
