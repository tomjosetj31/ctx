...
from spaceload.adapters.vpn.tailscale import TailscaleAdapter
from spaceload.adapters.vpn.wireguard import WireGuardAdapter
from spaceload.adapters.vpn.cisco import CiscoAnyConnectAdapter
from spaceload.adapters.vpn.mullvad import MullvadAdapter
from spaceload.adapters.vpn.openvpn import OpenVPNAdapter
...
class TestTailscaleAdapter:
    ...
    def test_connect_failure_returns_false(self):
        with patch("shutil.which", return_value="/usr/bin/tailscale"), \
            ...
            with patch("spaceload.adapters.vpn.base.time.sleep"):
                result = self.adapter.connect({})
...
class TestCiscoAnyConnectAdapter:
    ...
    def test_is_available_when_binary_exists(self, tmp_path):
        with patch("spaceload.adapters.vpn.cisco.Path") as mock_path_cls:
