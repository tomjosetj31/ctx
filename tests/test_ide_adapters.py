...
from spaceload.adapters.ide.vscode import VSCodeAdapter, _get_projects_from_storage as _get_vscode_projects
from spaceload.adapters.ide.cursor import CursorAdapter, _get_projects_from_storage as _get_cursor_projects
...
class TestVSCodeAdapter:
    ...
    def test_get_open_projects_reads_storage(self, tmp_path):
        ...
        with patch("spaceload.adapters.ide.vscode._get_projects_from_applescript", return_value=[]):
            with patch("spaceload.adapters.ide.vscode._STORAGE_CANDIDATES", [storage]):
    ...
    def test_get_open_projects_returns_empty_when_no_storage(self, tmp_path):
        ...
        with patch("spaceload.adapters.ide.vscode._get_projects_from_applescript", return_value=[]):
            with patch("spaceload.adapters.ide.vscode._STORAGE_CANDIDATES", [missing]):
    ...
    def test_get_open_projects_prefers_applescript(self, tmp_path):
        ...
        with patch("spaceload.adapters.ide.vscode._get_projects_from_applescript", return_value=[str(project_dir)]):
            result = self.adapter.get_open_projects()
        assert str(project_dir) in result
...
class TestCursorAdapter:
    ...
    def test_get_open_projects_reads_storage(self, tmp_path):
        ...
        with patch("spaceload.adapters.ide.cursor._get_projects_from_applescript", return_value=[]):
            with patch("spaceload.adapters.ide.cursor._STORAGE_PATH", storage):
                result = self.adapter.get_open_projects()
        assert str(project_dir) in result
    ...
    def test_get_open_projects_returns_empty_when_no_storage(self, tmp_path):
        missing = tmp_path / "missing.json"
        with patch("spaceload.adapters.ide.cursor._get_projects_from_applescript", return_value=[]):
            with patch("spaceload.adapters.ide.cursor._STORAGE_PATH", missing):
                assert self.adapter.get_open_projects() == []
...
class TestZedAdapter:
    ...
    def test_get_open_projects_reads_recent_projects(self, tmp_path):
        ...
        with patch("spaceload.adapters.ide.zed._RECENT_PROJECTS_PATH", recent):
            result = self.adapter.get_open_projects()
        assert str(project_dir) in result
    ...
    def test_get_open_projects_returns_empty_when_no_file(self, tmp_path):
        missing = tmp_path / "missing.json"
        with patch("spaceload.adapters.ide.zed._RECENT_PROJECTS_PATH", missing):
            assert self.adapter.get_open_projects() == []
